				...
				<div class="srk-scanner-access-note">
					...
					<div class="srk-scanner-access-copy">
						...
						<p>
							<?php
							printf(
								esc_html__( 'Free Link Scanner can scan Pages only up to %d links per run. Posts, whole website scans, and public custom post types require the $1.99 Broken Links + 404 Monitor add-on.', 'seo-repair-kit' ),
						...
						<td>
							...
							<p class="description">
								...
								echo $scanner_unlimited
									...
									: esc_html__( 'Whole Website coverage is available with the Broken Links + 404 Monitor add-on.', 'seo-repair-kit' );
								?>
							</p>
						</td>
					</tr>
							...
							<div class="srk-checkbox-group">
								<?php foreach ( $public_post_types as $pt_slug => $pt_obj ) : ?>
									...
									<label class="srk-checkbox-label <?php echo $is_allowed ? '' : 'srk-checkbox-label-disabled'; ?>">
										...
										<?php if ( ! $is_allowed ) : ?>
											<span class="srk-plan-lock"><?php esc_html_e( '$1.99 add-on', 'seo-repair-kit' ); ?></span>
										<?php endif; ?>
									</label>
								<?php endforeach; ?>
							</div>
