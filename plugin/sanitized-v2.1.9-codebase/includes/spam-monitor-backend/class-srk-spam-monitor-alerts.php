...
class SRK_Spam_Monitor_Alerts {
	...
	private static function format_issues( $issues ) {
		if ( is_string( $issues ) ) {
			return $issues ? sanitize_textarea_field( $issues ) : '-';
		}
	...
	private static function format_issue_lines( $issues ) {
		if ( is_string( $issues ) ) {
			$text = trim( sanitize_textarea_field( $issues ) );
		...
		if ( is_array( $evidence ) ) {
			...
			foreach ( $evidence as $key => $value ) {
				if ( is_scalar( $value ) ) {
					...
					$flattened[] = sanitize_text_field( $key ) . ': ' . implode( ', ', self::format_preview_list( array_map( 'sanitize_text_field', array_filter( $value, 'is_scalar' ) ), 6 ) );
				}
			}
