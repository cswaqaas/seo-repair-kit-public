...
Tags: spam monitor, meta manager, schema markup, 301 redirect, 404 monitor, broken link checker
...
**SEO Repair Kit v2.1.9** by [TorontoDigits](https://www.torontodigits.com) helps WordPress site owners find, monitor, and fix practical SEO issues from one dashboard.

It combines technical SEO checks, indexed-spam monitoring, metadata controls, crawling controls, Links Manager, automatic link scanning, 404 monitoring, smart redirects, notification emails, and Search Console insights, so you can improve visibility without relying on multiple plugins.

= Why SEO Repair Kit? =

Unlike single-purpose SEO plugins, SEO Repair Kit focuses on **practical SEO fixes and monitoring** in one place including indexed spam risks, broken links, 404 errors, redirects, missing alt text, metadata, schema, and search performance.
...
* Site owners who want to review suspicious indexed Google results
...
* Connect supported SERP providers such as Serper.dev, SERP API, and DataForSEO when the paid module is active
...
* Fix: Improved Bot Manager llms.txt frontend serving so saved llms.txt content is available at `/llms.txt`.
...
= 2.1.9 =
**Feature Update:** This release adds the Spam Monitor module for indexed Google SERP spam-risk monitoring, Spam Rules, scheduled scans, alerts, scan reports, and improved dashboard visibility. It also improves Bot Manager llms.txt serving at `/llms.txt`.
...
2. The dashboard provides:
   ...
   * Spam Monitor status and quick access when the module is available
...
1. Go to **SEO Repair Kit â†’ Sitemap Control**
...
Tip: If your sitemap is not opening, go to **Settings â†’ Permalinks** and click œSave Changes to refresh it.
...
2. Configure SEO using these tabs:
   ...
   * Respects WordPress **Settings â†’ Reading â†’ Discourage search engines** option
...
1. **Analytics & Reporting**:
   ...
   * Review Spam Monitor scan history, risky URL records, and alert history
...
3. **Integration**:
   ...
   * SERP provider support for Spam Monitor when the paid module is active
...
* If Spam Monitor scans do not run, verify the Spam Monitor module/provider status, schedule settings, and alert email configuration.
* If custom SERP provider settings are disabled, confirm the paid Spam Monitor module is active.
...
* If llms.txt redirects to the homepage, resave Bot Manager llms.txt content and confirm the site is running SEO Repair Kit 2.1.9 or newer.
...
= What is Spam Monitor? =
Spam Monitor is a SEO Repair Kit module for reviewing indexed Google SERP results, scoring spam risk, identifying suspicious URLs, and keeping a history of scans, alerts, and cleanup review items.

= What does Spam Monitor check? =
Spam Monitor can evaluate returned Google SERP records against Spam Rules such as unexpected language signals, spam keyword categories, suspicious URL patterns, and configurable risk thresholds.

= Can Spam Monitor run automatically? =
Yes. Scheduled Spam Monitoring can run on simple intervals such as daily, every 3 days, weekly, biweekly, or monthly. Manual scans remain available from the Google SERP Scan tab.

= Do I need my own SERP provider API key for Spam Monitor? =
Free users can use the SEO Repair Kit trial provider where available. The paid Spam Monitor module allows supported custom provider connections such as Serper.dev, SERP API, and DataForSEO.
...
Version 2.1.9 introduces the Spam Monitor module with dashboard stats, Spam Rules, Google SERP scanning, scheduled monitoring, alerts, scan reports, record pagination, export/clear table actions, and module status messaging. It also improves Bot Manager llms.txt serving.
